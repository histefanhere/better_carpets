...
execute unless entity @s[tag=bc_player_dest] run function bc:green/update_title
...
execute if entity @s[tag=bc_player_dest] unless entity @a[tag=on_red] run function bc:green/update_title

# 2 - There is already someone waiting on a red carpet to warp to us
#   - @a[tag=on_red] will succeed
#   - Begin their warping
execute if entity @s[tag=bc_player_dest] if entity @a[tag=on_red] as @a[tag=on_red] run function bc:red/start_warp
