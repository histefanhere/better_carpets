...
execute unless entity @s[tag=bc_item_dest] run function bc:carpets/special/lime/update_title
...
execute if entity @s[tag=bc_item_dest] unless entity @a[tag=on_orange] run function bc:carpets/special/lime/update_title

# 2 - There is already someone waiting on an orange carpet to send items to us
#   - @a[tag=on_orange] will succeed
#   - Begin item transfer
execute if entity @s[tag=bc_item_dest] if entity @a[tag=on_orange] as @a[tag=on_orange] run function bc:carpets/special/orange/start_warp
